        ...
        for p in range(len(x)):
            for q in range(len(y)):
                if np.sqrt(x[p].value**2 + y[q].value**2) <= PS_radius:
                    ...
                    pix_tmp = hp.ang2pix(
                        nside, x_ang.value / 180 * np.pi, y_ang.value / 180 * np.pi)
        ...
        for p in range(len(x)):
            for q in range(len(y)):
                ...
                if (DistFocus1 + DistFocus2 <= 2 * lobe_maj):
                    ...
                    # Judge and Fill
                    pix_tmp = hp.ang2pix(
                        nside, (theta + x_r).value / 180 * np.pi, (phi + y_r).value / 180 * np.pi)
        ...
        for p in range(len(x)):
            for q in range(len(y)):
                ...
                if (DistFocus1 + DistFocus2 <= 2 * lobe_maj):
                    ...
                    # Judge and Fill
                    pix_tmp = hp.ang2pix(
                        nside, (lobe2_theta + x_r).value / 180 * np.pi, (lobe2_phi + y_r).value / 180 * np.pi)
                    hpmap[pix_tmp] += PS_lobe[i]
        # Core
        pix_tmp = hp.ang2pix(nside, PS_data['Theta (deg)']/ 180 * np.pi, PS_data['Phi (deg)'] / 180 * np.pi)
...
def draw_ps(nside, Freq, FileName):
    """
    ...
    -------
    >>> Freq = [150,151,151]
    >>> hpmaps = draw_ps(nside = 512,Freq = Freq,FileName = 'PS_tables/FRI_20161011_100600.csv')
    """
    ...
    hpmaps = []
    # load csv
    ClassType, PS_data = read_csv(FileName)

    # get hpmaps
    if ClassType == 1 or ClassType == 2:
        for f in Freq:
            hpmap = draw_cir(nside, PS_data, ClassType, f)
            hpmaps.append(hpmap)
    elif ClassType == 3:
        for f in Freq:
            hpmap = draw_rq(nside, PS_data, f)
            hpmaps.append(hpmap)
    else:
        for f in Freq:
            hpmap = draw_lobe(nside, PS_data, ClassType, f)
            hpmaps.append(hpmap)
